...
> **⚠️ Breaking change in v0.7**: OAuth scopes have been renamed from `a2a.*` to `mcp:*`. Backward compatibility is preserved during the transition window, but update your integrations now. See the [scope migration guide](./scope-migration).
...
→ See the [OAuth 2.0 PKCE guide](./oauth-pkce) for the full implementation walkthrough.
