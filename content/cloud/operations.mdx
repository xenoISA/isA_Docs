...
make teardown-bigdata-kind
```

The setup flow preflights cert-manager CRDs before deploying the datalake stack. The verification flow checks readiness and smoke behavior for Kafka, PostgreSQL big-data metadata, Hive Metastore, MinIO, Iceberg tools, StarRocks, Apicurio Registry, Flink, Fluss, and Flink CDC jobs.
