...
> **RBAC**: When `RBAC_ENABLED=true`, each endpoint additionally requires a specific `resource:action` permission in the JWT. See [RBAC](./rbac) for the full permission matrix.
...
- Audio format: `pcm16`
- Turn detection: `server_vad` (threshold 0.5, silence duration 200ms)
...
```bash
curl -X POST http://localhost:8082/realtime/sessions/sess_abc123/interrupt
```
...
**Resolution priority:**
1. Explicit `model` + `provider` in request
2. Role-based defaults (see table below)
3. Capability-filtered candidates ranked by cost
...
See [Metering & Usage API](./metering) for full parameter reference and response schema.
...
See [Audit Logging](./audit-logging) for full parameter reference.
